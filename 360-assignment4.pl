...
%if it is true that X is less than Y
% and Xplus1 is assigned to the expression X +1
% then X is the head, Zs is the tail, process the
% value one greater than X
% keep going till limit of Y is reached
upto(X, X, [X]).
upto(X, Y, [X | Zs]) :-
    X < Y,
    Xplus1 is X + 1,
...
findpath(leaf(X), [X]).
findpath(node(R,X,_), [R|L]) :-
    findpath(X,L).
findpath(node(R,_,X), [R|L]) :-
    findpath(X,L).
